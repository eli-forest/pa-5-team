...
military <- read_xlsx("data/gov_spending_per_capita.xlsx",  #comment reads in data
                      sheet = "Share of Govt. spending",    #comment sheet name
                      skip  = 7,                            #comment doesn't load the first 7 rows
                      n_max = 191,                          #comment only loads up to row 191
                      na = c("xxx", ". ."))                  #comment added n/a to all blank boxes
```
...
sheet. We can use the `Region` column of this dataset to filter out the names we don't want.
...
> Its cleaning up the data by filtering the countries by pulling out from regions. 
...
military_long |>
  ...
  ggplot() +
  geom_boxplot(aes(x = spending, y = Country, color = Country))
...
**6. Looking at the plot you created above, which Eastern European country had the second highest median military expenditure?.**
 Armania has the second highest median military expenditure.
...
**7. Looking at the plot you created above, which Eastern European country had the largest variability in military expenditures over time?**
 Georgia has the largest variability.
